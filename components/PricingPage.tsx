...
const PricingPage = ({ onSelectFree }: { onSelectFree: () => void }) => {
  ...
  const plans = [
    {
      ...
      action: () => setShowPopup(true), // Local state popup
      popular: false
    },
    {
      name: "The Studio", 
      ...
      // ROUTE NAVIGATION TO CONTACT PAGE
  ...
  return (
    ...
    <div className="min-h-screen bg-[#FAFAFA] text-[#1A1A1A] py-32 px-6 overflow-hidden">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-stretch">
          {plans.map((plan, idx) => (
            ...
            >
              ...
              <button
                onClick={plan.action}
                className={`w-full py-6 rounded-2xl font-black uppercase tracking-widest text-[10px] transition-all flex items-center justify-center gap-3 group ${
                  plan.popular 
      ...
      {/* SUCCESS POPUP (FOR FREE PLAN ONLY) */}
      <AnimatePresence>
        {showPopup && (
          <motion.div 
            ...
            >
              ...
              <div className="space-y-4">
                <button 
                  ...
                  className="w-full bg-black text-white py-6 rounded-2xl font-black uppercase tracking-widest text-[11px] hover:scale-105 transition-all"
                >
                  Launch Builder
                </button>
                <button onClick={() => setShowPopup(false)} className="text-[10px] font-black uppercase text-zinc-300 hover:text-black tracking-[0.3em] transition-all">
                  Return to Plans
                </button>
              </div>
            </motion.div>
